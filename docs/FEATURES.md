...
| Portfolio performance metrics | Beta, Sharpe, alpha, max drawdown, win rate | **Partial** | Beta uses regression/SLOPE on date-aligned SPY and portfolio returns; cash flows are excluded and win-rate is real FIFO-matched P&L; risk-free rate is still hardcoded to 0 instead of configurable. `services/portfolio_metrics.py` |
