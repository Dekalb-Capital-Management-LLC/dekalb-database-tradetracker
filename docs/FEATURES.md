...
| Production deploy | Railway (API + seven-table Postgres) + Cloudflare Pages (frontend) | **Partial** | Railway now has bounded DB startup retries and a schema-aware `/health/ready` deployment gate; the feature-branch redeploy still needs a live Railway smoke test. Cloudflare/Google OAuth remain part of the end-to-end deployment check. `railway.toml`, `db.py`, `docs/DEPLOY_RAILWAY.md` |
